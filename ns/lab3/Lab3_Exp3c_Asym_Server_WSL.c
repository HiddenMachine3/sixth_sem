...
int main() {
    ...
    char *response2 = "Additional Message from server";
    unsigned char encrypted2[BUFFER_SIZE];
    encrypted_len = RSA_public_encrypt(strlen(response2), (unsigned char*)response2, encrypted2, rsa_pub, RSA_PKCS1_PADDING);
    if (encrypted_len == -1) { ERR_print_errors_fp(stderr); exit(EXIT_FAILURE); }
    
    write(new_socket, encrypted2, encrypted_len);
